...
public class PrinterNative {
  ...
  private static final byte[] INIT_COMMAND = {
    (byte) 0x1B,
    (byte) 0x40, // ESC @ Initialize printer
  };

  private static final byte[] FEED_COMMAND = {
    (byte) 0x1B,
    (byte) 0x64,
    (byte) 0x05, // ESC d n - Feed n lines
  };
  ...
  private static final byte[] THAI_CHARACTER_CODE = {
    (byte) 0x1B,
    (byte) 0x74,
    (byte) 0x0F, // ESC t 15 - Select Thai Character Code
  };

  private static final byte[] SET_UTF8_MODE = {
    (byte) 0x1C,
    (byte) 0x43,
    (byte) 0x01, // FS C 1 - Enable UTF-8
  };

  private static final byte[] SET_CODEPAGE_UTF8 = {
    (byte) 0x1B,
    (byte) 0x74,
    (byte) 0x03, // ESC t 3 - Select UTF-8 codepage
  };
  ...
  private static final byte[] SET_MAX_SPEED = {
    (byte) 0x1B,
    (byte) 0x73,
    (byte) 0x00, // Set maximum speed
  };

  private static final byte[] PRINT_DENSITY = {
    (byte) 0x1B,
    (byte) 0x6D, // Set print density to high
  };
  ...
  public static String[] getAllSerialPorts() {
    ...
    for (File file : files) {
      ...
      if (name.matches("tty(S|USB|ACM)[0-9]+")) {
        serialPorts.add("/dev/" + name);
      }
    }
  ...
  // เพิ่มใน PrinterNative.java
  public boolean isPortCurrentlyOpen(String portPath, int baudRate) {
    // สมมติว่า PrinterNative เก็บ currentPortPath และ currentBaudRate เหมือน WeightReader
    // คุณอาจต้องเพิ่มตัวแปรเหล่านี้ใน PrinterNative ถ้ายังไม่มี
    return (
      this.output != null &&
  ...
  public void closePort() {
    try {
      ...
      currentPortPath = null; // **เพิ่มบรรทัดนี้**
      currentBaudRate = 0; // **เพิ่มบรรทัดนี้**
    } catch (Exception e) {
